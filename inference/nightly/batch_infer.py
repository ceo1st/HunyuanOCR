...
from hunyuan_tasks import TASK_PROMPTS, DEFAULT_TASK, get_prompt

EXTS = (".png", ".jpg", ".jpeg", ".webp", ".bmp")


def run_one(client, model, img_path, prompt, max_tokens, rep_pen, repeat_min,
            do_doc_pp):
    messages = [
        {"role": "system", "content": ""},
        {"role": "user", "content": [
    ...
    common = dict(
        ...
        extra_body={"top_k": -1, "repetition_penalty": rep_pen,
...
def main():
    ...
    ap.add_argument("--no-doc-postprocess", action="store_true",
                    help="doc_parse 时关闭 markdown 规整")
    ...
    clients = [OpenAI(api_key="EMPTY", base_url=f"http://{args.host}:{p}/v1", timeout=3600.0)
               for p in ports]
    prompt = get_prompt(args.task_type)
    do_doc_pp = (args.task_type == "doc_parse") and (not args.no_doc_postprocess)
    print(f"[info] task={args.task_type} doc_postprocess={do_doc_pp} "
    ...
    def work(item):
        ...
        try:
            t = time.time()
            text, early, pp = run_one(client, args.model, img_path, prompt,
                                      args.max_tokens, args.repetition_penalty,
                                      args.repeat_min_repeats, do_doc_pp)
            with open(md, "w", encoding="utf-8") as f:
                f.write(text)
            pp_applied = bool(pp and any(v for v in pp.values()))
            return {"image": fname, "chars": len(text), "early_stopped": early,
                    "doc_pp": {k: v for k, v in (pp or {}).items() if v},
                    "latency": round(time.time() - t, 2), "ok": True,
    ...
    with ThreadPoolExecutor(max_workers=args.concurrency) as ex:
        ...
        for fut in as_completed(futs):
            ...
            if not r["ok"] or st["n"] % 25 == 0:
                el = time.time() - st["t0"]; rate = st["n"] / el if el else 0
                print(f"[{st['n']}/{len(todo)}] ok={st['ok']} err={st['err']} "
                      ...
                      flush=True)
    jsonl.close()
    print(f"[done] ok={st['ok']} err={st['err']} early={st['early']} "
          f"doc_pp_applied={st['pp']} in {(time.time()-st['t0'])/60:.1f}min", flush=True)
