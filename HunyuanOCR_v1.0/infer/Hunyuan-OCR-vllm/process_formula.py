...
def clean_formula(text: str):
    return text.replace("\\quad", "").replace("$", "").strip()
...
def process_badcases(Form_part2):
    ...
    for case in Form_part2:
        ...
        results.append({"img_id": case["img_id"], "gt": gt, "pred": pred, "match_score": score})
...
def process_formula_matching(match_file, markdown_file, markdown_key, output_file):
    ...
    for idx, item in enumerate(raw_data):
        ref = item["gt"].replace("$", "").replace("\[", "").replace("\]", "").replace("，", ",").strip()

        pred = item["pred"].replace("$", "").replace("\[", "").replace("\]", "").replace("，", ",").strip()

        final_ans.append({"img_id": f"{idx}", "id": f"{item['img_id']}", "gt": ref, "pred": pred, "edit": item["edit"]})
    ...
    Form_part2 = [
        x for x in error_match_ans if x["id"] == "yanbaopptmerge_9081a70ff98b3e7d640660a9412c447d.pdf_1287.jpg"
    ]
